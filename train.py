...
parser.add_argument('-save_path', type=str, help='save path', default='/ws/external/models/baseline/')
...
def main():
    ...
    logger = Logger(args.save_path + "stdout.log")
    logger.write('Start Training\nInitiliazing\n')
    logger.write('src: ' + args.source)
    logger.write('tar:' + args.target)
    # print('Start Training\nInitiliazing\n')
    # print('src:', args.source)
    # print('tar:', args.target)
    # source_train_dataset = BatchPaddingData(pc_input_num=2048, status='train', aug=True, swapax=swapax, pc_root='/ws/data/%s' % args.source, batch_size=32, sample_num=task_num)
    source_train_dataset = PaddingData(pc_input_num=2048, status='train', aug=True, swapax=swapax, pc_root='/ws/data/%s' % args.source)
    source_test_dataset = PaddingData(pc_input_num=2048, status='test', aug=False, swapax=swapax, pc_root='/ws/data/%s' % args.source)
    target_test_dataset = PaddingData(pc_input_num=2048, status='test', aug=False, pc_root='/ws/data/%s' % args.target)

    test_datasets = [
        PaddingData(pc_input_num=2048, status='test', aug=False, drop=0.24, swapax=swapax, pc_root='/ws/data/%s' % args.source),
        PaddingData(pc_input_num=2048, status='test', aug=False, drop=0.36, swapax=swapax, pc_root='/ws/data/%s' % args.source),
        PaddingData(pc_input_num=2048, status='test', aug=False, drop=0.45, swapax=swapax, pc_root='/ws/data/%s' % args.source),

        PaddingData(pc_input_num=2048, status='test', aug=False, p_scan=0.017, swapax=swapax, pc_root='/ws/data/%s' % args.source),
        PaddingData(pc_input_num=2048, status='test', aug=False, p_scan=0.022, swapax=swapax, pc_root='/ws/data/%s' % args.source),
        PaddingData(pc_input_num=2048, status='test', aug=False, p_scan=0.035, swapax=swapax, pc_root='/ws/data/%s' % args.source),

        PaddingData(pc_input_num=2048, status='test', aug=False, density=1.3, swapax=swapax, pc_root='/ws/data/%s' % args.source),
        PaddingData(pc_input_num=2048, status='test', aug=False, density=1.4, swapax=swapax, pc_root='/ws/data/%s' % args.source),
        PaddingData(pc_input_num=2048, status='test', aug=False, density=1.6, swapax=swapax, pc_root='/ws/data/%s' % args.source),

        PaddingData(pc_input_num=2048, status='test', aug=False, swapax=swapax, pc_root='/ws/data/%s' % args.source),
    ...
    source_train_dataloader = DataLoader(source_train_dataset, batch_size=args.batchsize, shuffle=True, num_workers=4, drop_last=True)
    ...
    model = Pointnet_cls(num_class=num_class)
    ...
    for epoch in range(max_epoch):
        ...
        for batch_idx, batch in enumerate(source_train_dataloader):
            ...
            logits = model(data, pn)
            loss = F.cross_entropy(logits, label)
            with torch.no_grad():
                accuracy = (torch.argmax(logits, dim=1) == label).float().mean()
            loss.backward()
            optimizer.step()

            if (batch_idx + 1) % 10 == 0:
                logger.write('Train:{} [{} /{}  loss: {:.4f} accuracy: {:.4f}]\n'.format(
                epoch, batch_idx, num_source_train, loss.item(), accuracy.item()))
                # print('Train:{} [{} /{}  loss: {:.4f} accuracy: {:.4f}\t]'.format(
                # epoch, batch_idx, num_source_train, loss.item(), accuracy.item()))

        lr_schedule.step()
        logger.write('Current lr: {}\n'.format(lr_schedule.get_last_lr()[0]))
        ...
        with torch.no_grad():
            ...
            if pred_acc > best_source_test_acc:
                ...
                save_path = args.save_path + "model2scan_best_source.pt"
                torch.save(model.state_dict(), save_path)

            logger.write('Source Test:{} [overall_acc: {:.4f} \t loss: {:.4f} \t Best Source Test Acc: {:.4f}]\n'.format(
            ...
            if pred_acc > best_target_test_acc:
                ...
                save_path = args.save_path + "model2scan_best_target.pt"
                torch.save(model.state_dict(), save_path)
            logger.write('Target:{} [overall_acc: {:.4f} \t loss: {:.4f} \t Best Target 2 Acc: {:.4f}]\n'.format(
        ...
        # save_path = args.save_path + "model2scan_e{}.pt".format(epoch)
        # torch.save(model.state_dict(), save_path)
        logger.write('Task acc: {}'.format(task_acc))
        logger.write('task loss: {}'.format(task_loss))
